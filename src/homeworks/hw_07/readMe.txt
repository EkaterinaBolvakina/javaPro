### Assignment: Develop a framework for a working application

#### Objective:
To develop a framework for a working application, which is a mini-project.
In the course of the assignment you need to create the main components of the application: entity, repository, service and controller.

#### Job Description:
You need to create an application that will perform CRUD (Create, Read, Update, Delete) operations for a specific entity using collections to store data.
The application should allow you to add, edit, delete and view objects of this entity.

#### Requirements:

1. **Entity**:
    - Create a class that represents an entity in the system (e.g. `Product`, `Customer`, `Order`, etc.).
    - Example attributes for the `Product` class:
...
2. **Repository**:
    - Create a `YourEntityRepository` interface that contains methods to work with a collection of objects.
    - The methods of the `YourEntityRepository` interface are:
        - `List<YourEntity> findAll()`.
        - `YourEntity findById(Long id)`.
        - `YourEntity save(YourEntity entity)`
        - `void deleteById(Long id)`
        - Additional methods for searching by various criteria (e.g., `List<YourEntity> findByName(String name)`).

3. **Service**:
    - Create an interface `YourEntityService` and its implementation `YourEntityServiceImpl` that will use a collection to store data.
    - Methods of the `YourEntityService` interface:
        - `List<YourEntity> findAll()`.
        - `YourEntity findById(Long id)`
        - `YourEntity save(YourEntity entity)`.
        - `void deleteById(Long id)`
...
4. **Controller**:
    - Create a `YourEntityController` class labelled with the `@RestController` annotation.
    - Configure routes to handle the following HTTP requests:
        - `GET /entities` - get a list of all objects
        - `GET /entities/{id}` - get object by identifier
        - `POST /entities` - add new object
        - `PUT /entities/{id}` - update an existing object
        - `DELETE /entities/{id}` - delete an object by identifier
        - Additional routes for searching by different criteria (e.g. `GET /entities/search?name={name}`).

#### Evaluation Criteria:
1. Correctness and completeness of the implementation of the `YourEntity`, `YourEntityRepository`, `YourEntityService`, `YourEntityServiceImpl` and `YourEntityController` classes.
2. Workability of CRUD operations.
3. Testing of the created application (you can use JUnit to write tests).
4. Cleanliness and readability of the code.
